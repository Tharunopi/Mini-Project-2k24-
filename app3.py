    ...
    for x, y in precautions.iterrows():
        for i in real:
            if y["Disease"] == i:
                ...
                pre_li.append(f"3. {c}")
                pre_li.append(f"4. {d}")
    return jsonify({"disease": real, "accuracy": accurcy, "discription": desc, "diet": dit, "precaution": pre_li})
...
def process_audio():
    ...
    real = []
    diets = pd.read_csv("E:\medicine\diets.csv")
    ...
    for x in severity:
        for i in real:
            if x[2] == i:
                workout_info.append(x[3])
    medicine_info = []
    for i in med["Ayurveda Medication"]:
        if i == real:
            medicine_info.append(i)
    ...
    for x, y in disease:
        for i in result:
            if x == i:
                real.append(y)
    return render_template('second-template.html', transcription=text, result=real, accuracy=accurcy, reason=re,medicine="", diet=dit, workout=workout_info)
    # return render_template('about.html', transcription=text, result=real, accurcy=accurcy,  diet=dit, workout=workout_info, medicine=medicine_info, reason=re)
